...
const CategoryProvider = ({ children }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://hanibackendapp.onrender.com/api/category/newcat`, {
        method: "POST",
        headers: {
  ...
  const getAllCategory = async () => {
    try {
      ...
      const response = await fetch(
        "https://hanibackendapp.onrender.com/api/category/allcategory",
        {
  ...
  return (
    <Category.Provider
      value={{ handleFields, state, handleSubmit, handleSelect,getAllCategory }}
    >
      {children}
    </Category.Provider>
  );
